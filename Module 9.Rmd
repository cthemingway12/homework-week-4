...
CONFIDENCE INTERVAL of 95% represents the central 95% of the data around the mean that we collect. We can use the statistic that we are measuring (ex. mean) + or - some critical value x the standard error

STANDARD DEVIATION tells you how variable the data in your sample is

STANDARD ERROR tells you how variable you expect the data to be in the population based on your sample. 

CENTRAL LIMIT THEORUM the distribution of some sample goes to normal as the sample size increases. Doesn't say anything about the distribution of your sample. 

Example simulation:
lambda = 4
standard deviation and mean are also both = 4
take individual samples of 10 from the population
...
#could also do this for samples of 1000; with the shrinking variation around the mean; as n goes to infinity, standard error should go to zero. 
n<-100
pop_se<-sqrt(lambda/n)
pop_se
...
#here the standard deviation is shrinking as the sample size increases. 
...
z<- (x -lambda)/pop_se
hist(z, breaks = seq(from = -4, to =4, length.out = 20), propability = TRUE, add = TRUE)

#could also do this with other summary statistics such as sum. 
```

Summary:
-CLT says regaurdless of underlying distribution, the distribution of averages based on a large number of indpenedent, identically distributed cariables will be approximately normal, centeral at the population mean, and with the standard deication equal to the stand error of the mean. 
...
**p value** = the probability of our obtaining that test statistic or a more extreme one by chance, if the null hypothesis were true.

The propability of falsly rejecting the null hypothesis based on your data is 5% or alpha=0.05. 
...
Suppose we have a vector describing the adult weights of vervet monkeys trapped in South Africa during the 2015 trapping season. We have the sense they are heavier than vervets we trapped in previous years, which averaged 4.9 kilograms. The mean is 5.324 kilograms. Is the mean significantly greater than our expectation based on previous trapping history?
...
- What is our Ho?
the mean from our sample is no different from our expected mean
- What is our Ha?
the mean from our sample is signifcantly heavier than our expected mean
- What is the hypothesis we want to test? Is it two-tailed? Upper-tailed? Lower-tailed?
this is a one-tailed, upper-tailed hypothesis
- Calculate the mean, standard deviation, and SEM from our sample

``` {r}
mu <- 4.9 #expected mean
x <- d$weight #vector of interest (new data)
m <- mean(x) #mean of that vector (mean of new data)
s <- sd(x) #standard deviation of that vector (sd of new data)
n <- length(x) #number of samples that we have collected
sem <- s/sqrt(n) #standard error based on new data
```
...
z = (mean of our sample - expected mean)/(standard error of the mean)
z = (m - mu)/(sem)
...
z
```
This represetns where our population lies in our expected distribution. What this tells us is: our data falls 3 standard deviation from the expected mean. 
...
pnorm tells us the probability associated with a particular quantile (in this case 3.1 is the quantile for our new mean)
...
p <- 1 - pnorm(z)
p
p <- pnorm(z, lower.tail = FALSE) #same thing as above but here specifying that we want the upper tail 
p
```
So, if we are using a normal distribution for our statistical test, this different would be considered significant. 

Technially there is a 1/1000 chance of getting this number as the sample mean using this distribution. 
...
``` {r}
p <- 1 - pt(z, df = n-1) #using pt command now because we are using a t distribution instead of a normal distribution
p
p <- pt(z, df = n-1, lower.tail = FALSE) 
p
```
There is a slightly higher p-value here but it is still super significant. Still less than 2 chances in 1000. 
...
An example of this would be comparing males and females. 
...
Now using a pooeled variance of both samples -- this is only possible if the two variances are equal. 

This t statistic is almost the same as before when we assumed that they were unequal variances. This test has far fewer degrees of freedom. 
...
var(x)/var(y)
```
To figure out whether you should pool your data based on variance, just look at a ratio of the two variances. If this ratio is greater than 2 -> don't use the equal variance test!
